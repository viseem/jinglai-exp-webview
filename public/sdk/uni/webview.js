!(function (e, n) {
	...
	let r = function (e, n) {
			let i = { options: { timestamp: +new Date() }, name: e, arg: n }
			if (a()) {
				if ('postMessage' === e) {
					let r = { data: n }
				...
				if (0 === t.length) {
					let u = plus.webview.currentWebview()
	...
	let _ =
		...
		/miniProgram/i.test(navigator.userAgent)
	let m = window.qa && /quickapp/i.test(navigator.userAgent)
	let f =
		window.ks &&
	...
	S || (S = {})
	let P = 'undefined' != typeof uni ? uni : {}
	if (!P.navigateTo) for (let b in S) i(S, b) && (P[b] = S[b])
